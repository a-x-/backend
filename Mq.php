...
class AlxMq extends Mq
{
    ...
    protected function parse($reqLine, &$sigma, &$params, $isLog = false)
    {
        ...
        foreach ($reqArr as $req) { // Проход по отдельным alx-запросам
            ...
            $part[3] = preg_replace('!(?:^|\W)COUNT([^\(\w]|$)!i', "COUNT($part[1].id) AS `COUNT`$1", $part[3]); //  Замена COUNT на COUNT(PrimaryTable.id)
            $part[3] = preg_replace('!(?:^|\W)COUNT\s*\(([^\.]*?)\)!i', "COUNT($1.id) AS `COUNT`", $part[3]); //      Замена COUNT(SomeTable) на COUNT(SomeTable.id)
